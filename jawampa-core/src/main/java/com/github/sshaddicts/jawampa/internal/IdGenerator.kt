...
object IdGenerator {
    ...
    /**
     * Generates a new ID through a random generator.<br></br>
     ...
     * @return The new ID
     */
    fun newRandomId(controlMap: Map<Long, *>?): Long {
        while (true) {
            val l = ThreadLocalRandom.current().nextLong()
    ...
    /**
     * Generates a new ID by incrementing an old one.<br></br>
     * If the new ID is not valid or is already in use as a key in the provided Map
     * the process will be repeated until a valid iD can be returned.
     ...
     * @return The new ID
     */
    fun newLinearId(startValue: Long, controlMap: Map<Long, *>?): Long {
        var `val` = startValue
        while (true) {
            `val`++
