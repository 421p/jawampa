...
import com.github.sshaddicts.jawampa.connection.IWampConnectionListener

/**
 * Connection listener for that receives and processes WampMessages and state
 * events from the pipeline.<br></br>
 * A new instance of this is created for each connection attempt.<br></br>
 ...
 */
internal class ClientConnectionListener(private val stateController: StateController) : IWampConnectionListener {
    ...
    override fun transportClosed() {
        // Remark: No need to check for connection. The controller guarantees that after close()
        // messages will no longer be forwarded
        transportError(ApplicationError(ApplicationError.TRANSPORT_CLOSED))
    }
